...
class TestInvestopediaApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.broker = Account(self.USERNAME, self.PASSWORD)
    ...
    def test_buy(self):
        self.assertTrue(self.broker.trade(self.SYMBOL, Action.buy, 1))

    def test_sell(self):
        portfolio = self.broker.get_current_securities()
        for bought in portfolio.bought:
            if bought.symbol == self.SYMBOL:
                self.assertTrue(self.broker.trade(self.SYMBOL, Action.sell, 1))
                return
        self.assertTrue(False,
                            'Possesses no stock to test TestInvestopediaApi.test_sell(). Buy some manually.')

    def test_get_current_securities(self):
        self.assertTrue(self.broker.get_current_securities())
